...
export function useChessMatch(matchId: number) {
  ...
  const [isAutoPlay, setIsAutoPlay] = useState(true);
  const [timeControl, setTimeControl] = useState("5+0"); // 5 minutes fixed for the display
  const [increment, setIncrement] = useState(0); // Time increment in seconds, fixed to 0
  ...
  useEffect(() => {
    ...
    // Only run the timer if the game is in progress
    if (
      status === "inProgress" &&
      ...
      timerIntervalRef.current = setInterval(() => {
        setTimers((prev) => {
          ...
          const newTime = activeTimer ? Math.max(0, prev[activeTimer] - 1) : 0;

          // Check for time forfeit
          if (activeTimer && newTime === 0) {
            // Stop the timer
            if (timerIntervalRef.current) {
              clearInterval(timerIntervalRef.current);
              timerIntervalRef.current = null;
          ...
          return { ...prev, ...(activeTimer ? {[activeTimer]: newTime} : {}) };
        });
      }, 1000);
    }
  ...
  }, [status, matchData?.status, updateMatchStatus, activeTimer]);
  ...
  useEffect(() => {
    if (matchData) {
        // We set the fixed time control in all matches.
        setTimeControl("5+0");
        // We set the timers fixed for all matches.
        setTimers({
            white: 300,
            black: 300,
        });
        // We fix the increment to 0.
        setIncrement(0);
...
rement] = useState(0); // Time increment in seconds, fixed to 0
  const [activeTimer, setActiveTimer] = useState<"white" | "black" | null>(
    null
  );
