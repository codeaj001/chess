...
function UpcomingMatchCard({ match, onClick }: UpcomingMatchCardProps) {
	...
	const formattedTime = formatDistanceToNow(new Date(match.start_time), {
		addSuffix: true,
	});
	const timeFormatted = formattedTime.startsWith("in ")
		? formattedTime
		: `in ${formattedTime}`;
	...
	return (
		...
		>
			...
			<div className="flex justify-between items-center text-xs text-gray-400">
				...
				<span
					className={`px-2 py-0.5 rounded-full ${getMatchTypeBadge(
						match.match_type
					)}`}
				>
					{match.match_type}
...
export function MatchSchedule({
	...
	const { data: upcomingMatches, isLoading } = useQuery({
		queryKey: ["/api/matches/upcoming", 5], // Limit to 5 matches
	});
	...
	return (
		<div className="glassmorphism rounded-xl p-4">
			...
			<Button
				variant="secondary"
				className="w-full mt-4 py-2 px-4 bg-white/10 hover:bg-white/15 transition flex items-center justify-center"
