...
function MatchCard({ match, onClick }: MatchCardProps) {
  ...
  if (match.status === "Scheduled") {
    ...
    if (isAfter(startTime, new Date())) {
      timeDisplay = `Starts ${formatDistanceToNow(startTime, { addSuffix: true })}`;
    } else {
    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex flex-col items-end">
          ...
          <Badge className={`mt-1 ${getMatchStatusBadge(match.status)}`}>
            {match.status}
          </Badge>
        </div>
      </div>
...
export function MatchList({ onSelectMatch }: { onSelectMatch: (matchId: number) => void }) {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>View and manage AI chess matches</CardDescription>
      </CardHeader>
